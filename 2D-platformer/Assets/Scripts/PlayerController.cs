...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Attack
        if (Input.GetMouseButtonDown(0) && !isAttacking && Time.time >= lastAttackTime + currentWeapon.hitCooldown)
        {
            isAttacking = true;
    ...
    public void Jump(InputAction.CallbackContext context)
    {
        ...
        {
            isJumping = true; isWallJumping = true;
            rb.linearVelocity = new Vector2(wallJumpDirection * 5f, 10f);
            wallJumpTimer = 0; jumpFX();
            if ((isFacingRight && wallJumpDirection < 0) || (!isFacingRight && wallJumpDirection > 0)) Flip();
        ...
        if (context.performed)
        {
            ...
            if (canJump)
            {
                ...
                jumpsRemaining--; coyoteTimeCounter = 0f;
                jumpFX();
            }
        }
